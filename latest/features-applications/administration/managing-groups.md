...
When the Federated Catalog option is activated, you can create Explorer groups to give a user read access to one or several catalogs.
  ...
  ![](./images/group-explorer-type.png)
...
This permission allows users to manage agents from the **Agents** tab. Users can enable or disable agents, edit agent settings, and manage which groups can use them. For more information, see [Managing Agents](./managing-agents.md).
...
The agent must also be enabled by an administrator before any group can use it. For more information, see [Managing Agents](./managing-agents.md).
