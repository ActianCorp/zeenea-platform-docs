...
* **Name**: The name of the Item defined in Studio or by API.
* **Description**: Can be edited in Studio or by API
...
* **Last update**: Date of last update of Item documentation in Studio. For Items originating from connections, this date is updated each time a new inventory or synchronization is created, or for datasets each time the schema is verified.

When an Item comes from a connection (e.g., Dataset), it also includes other attributes from this connection. These attributes are read-only in Actian Data Intelligence:
...
In Actian Data Intelligence, each type of Item can have specific attributes, depending on its nature:
...
Only certain user profiles can edit an Item's documentation from within Studio. To do so, a user must meet one or both of the following conditions, depending on his or her user groups:
...
For Items imported from a connection, it is possible to use the description defined at the data source level, rather than defining a description manually in Actian Data Intelligence.
...
Whatever its origin or format, a summary is generated by Actian Data Intelligence when each new description is created.
...
2. Click the tag property field. <br />A dropdown list of available tag values appears.
...
For more information about configuring the glossary metamodel, see [Configuring the Glossary Metamodel](../../studio/catalog-design/studio-configure-glossary-model.md).

## User Suggestions

In Explorer, users can submit suggestions to improve the documentation of an Object, or to report an error. Suggestions for a given Object are listed in the General tab of its details page, in the **Suggestions** section.

![](./images/user-suggestions.png)
...
For more information about submitting a suggestion, see [Submitting a Suggestion](../../explorer/submit-suggestion.md).
