...
!!! warning "Microsoft Fabric Connector Recommended"
    For new integrations, use the **Microsoft Fabric** connector instead of **Power BI Online (V2)**. The Microsoft Fabric connector covers the full Power BI Online (V2) scope and includes additional improvements. For more information, see [Adding a Microsoft Fabric Connection](../ms-fabric/ms-fabric.md).
...
In addition, to retrieve lineage information, `dataRetrievalState` must not include `DatasetExpressionsDisabledByAdmin`.
