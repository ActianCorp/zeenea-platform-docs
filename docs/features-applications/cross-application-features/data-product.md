...
* Synchronize data products and their data contracts from your CI/CD pipelines by using the dedicated [Data Product API](../../technical-documentation/api/data-product-api.md).
...
These YAML files can be uploaded to the platform through a dedicated [REST API](../../technical-documentation/api/data-product-api.md). This API can be called from external tools or the CI/CD pipelines, for example, from a GitHub Action, as illustrated in the following diagram:

![](./images/data-product-api.png)
...
![](./images/data-product-output-port.png)
...
![](./images/data-product-data-quality-status.png)
...
![](./images/data-product-search-data-products.png)

### Request Access to a Data Product

In Explorer, users can request access to data products at the output port level.

To enable access requests for data product output ports, create an [access request policy](../cross-application-features/access-requests.md) in Administration. Then assign the access request policy as follows:
