...
The Databricks Unity Catalog connector is compatible with REST API versions `2.0` and `2.1`. 

## Installing the Plugin

You can download the Databricks Unity Catalog plugin from [Zeenea Connector Downloads](./zeenea-connectors-list.md).
...
| `lineage.enabled` | Activates the lineage feature. Default value is `false`. |
...
To extract information, the connector runs REST requests on the following endpoints:
...
* **GET**: `/api/2.0/lineage-tracking/table-lineage`: To get table lineage from jobs.
* **GET**: `/api/2.1/jobs/get`: To get job metadata.
...
The connector can retrieve Databricks jobs executed on a Databricks instance when the lineage parameter is enabled. It tracks lineage only between Databricks tables.
...
A data process represents a Databricks job.
...
| Data Process | code/catalog/schema/dataset name/job_id | - **code**: Unique identifier of the connection noted in the configuration file<br/>- **catalog**: Object catalog<br/>- **schema**: Object schema<br/>- **dataset name**: Table or view name<br/>- **job id**: Job internal ID |
