...
!!! note
    Information required for this integration may be provided by both Actian and the Customer.
...
### Actian Data Intelligence Credentials
...
* Certificate: Actian may provide the certificate (1) that will decrypt requests sent by Auth0 to the Identity Provider (hereafter called “IdP”).
...
!!! note
    In the example above, “CONNECTION” refers to the name of your Actian Data Intelligence instance. This connection **MUST** have been created before requesting this resource otherwise, the metadata file won't be accessible.
...
|---|---|---|
| Connection Name | Actian | Name of the connection, set up by Actian. It represents your instance. |
...
| Sign in URL | Customer.<br/><br/>Required. Must be given to Actian. | Unique login URL for SAML |
| X.509 Certificate | Customer.<br/><br/>Required. Must be given to Actian. | Certificate (provided by the customer) to be uploaded. |
| Sign Out URL | Customer.<br/><br/>Required. Must be given to Actian. | Unique logout URL for SAML |
...
| Debug Mode | Actian | To be toggled on or off |
| Sign Request | Customer.<br/><br/>Should the request be signed? | When enabled, the SAML authentication request will be signed. Actian will then provide the customer with a decrypting certificate (1). |
...
The signature of the assertions/tokens that prove the identity of a given end-user account managed by Auth0 relies on a self-signed certificate, of type x.509 v3, key size 2048 bits, created by Auth0, and not a certificate issued by a certification authority. This does not pose any security problem, because the trust relationship between the issuer of the assertions/tokens (Auth0) and the consumer (Actian's client) is manually configured. In addition, the self-signed certificate provided by Auth0 has a long lifetime (current certificate expires at the end of 2033), so from a lifetime perspective, there will be no need to frequently change the certificate.

Finally, you explicitly configure your IdP to trust the identity of users from this specific Auth0 certificate that you obtained from Actian and that was manually provided to you. So this trust happens to be based on a certificate, but the fact that this certificate is issued by a certificate authority would not add anything in terms of trust relationship.
...
In any case, the attribute containing the email must be communicated to Actian; this will allow us to correct the setup (if needed), and to make sure that the information is correctly retrieved. 
...
This kind of connection does not require any information from your end. It is set up entirely by Actian.