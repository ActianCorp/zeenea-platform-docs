...
# Actian Data Intelligence Scanner Setup

## Architecture Overview

![](./images/scanner-architecture.png)

* Each instance of Actian Data Intelligence Scanner must be deployed in your infrastructure.
...
* The Scanner communicates with the Actian Data Intelligence Platform only on its own initiative: no incoming feed is required.
* The communication between the Scanner and the Actian Data Intelligence Platform is done via the HTTPS protocol (on default port: 443), in a secure way (TLS) and via an authentication based on an API key generated by you.
* This outgoing communication can optionally pass through a proxy.

!!! warning "Required — The scanner must keep running"

    The Actian Data Intelligence Scanner must remain running for metadata import and synchronization to work. It is a long-running background process, not a one-time command.

    * Keep the scanner running continuously — during the execution of its sub-processes (by default, every night) and whenever users access the Actian Data Intelligence Platform. For production environments, run the scanner as a managed service to ensure that it remains running and restarts automatically (see [Process Configuration in systemd](#process-configuration-in-systemd)).
...
Actian Data Intelligence Scanners must be installed on x86-64 Linux servers or Windows servers.
...
The Actian Data Intelligence Scanner process will establish connections to the systems it is connected to and from which it will need to extract metadata.

Depending on the platforms, the protocols will vary and the flow openings will have to be carried out accordingly.

The Actian Data Intelligence Scanner process will also exchange with the Actian Data Intelligence SaaS platform. The network flow between the Actian Data Intelligence Scanner process and the platform will therefore have to be allowed. The exchanges are always initiated by the Scanner process and are done in HTTPS. A proxy (with a possible authentication) can be configured.

### Necessary Local Rights

The Actian Data Intelligence Scanner service must be run with a dedicated user account (no login shell user account).
...
Unless the binary is specifically provided to you by Actian, the Scanner can be downloaded as an archive by using a dedicated API entry point or via the Scanners monitoring page in Actian Data Intelligence (Administration interface) where a download link is proposed.

If you decide to download the Scanner using the API, you have to create a dedicated API key. See [Managing API Keys](../../features-applications/administration/managing-api-keys.md).
...
The name will be the identifier of the Scanner. It will be used by Actian Data Intelligence to distinctly identify the Scanner and allow the Connections associated with it to be identified.
...
You must enter the address of your Actian Data Intelligence Platform to allow the Scanner to retrieve the metadata. This is done by enhancing the `zeenea-url` property with the URL of your platform in the form:
...
The Scanner authenticates itself to the platform with an "Scanner" type API key containing a pair of information (id and secret). To generate an API key, refer to [Managing API Keys](../../features-applications/administration/managing-api-keys.md).
...
* **synchronize**: Some connectors don't use an inventory to let the Data Steward import the items they want, but instead synchronize automatically all the items from the source system with Actian Data Intelligence Platform. This process will trigger these connectors. By default, this process runs every 24H.
* **collect-fingerprint**: Compute fingerprint and data profiling metrics for items with this option activated. By default, this process runs once a week.
...
It is possible to configure a proxy that will be used by the Scanner during exchanges with the Actian Data Intelligence Platform.
...
```
After you save the changes, restart the scanner. The scanner then uses gRPC to communicate with your Actian Data Intelligence Platform.
...
For more about our connectors, see [Connector Downloads](../connectors/connectors-list.md).

New connections are defined using a dedicated configuration file under the connections folder of the Scanner.

Learn how to [create, manage or delete a connection](../../features-applications/administration/managing-connections.md).
...
For more information about configuring a hook after a scanner job, see [Configuring a Hook after a Scanner Job](configure-hook.md).
...
!!! note
    ...
    * The path to Actian Data Intelligence must match or be adapted.
...
If you experience issues while configuring your scanner, refer to [Troubleshooting Scanners and Connections](./troubleshooting.md)
