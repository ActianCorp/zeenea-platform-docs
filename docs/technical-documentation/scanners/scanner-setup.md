...
* The communication between the Scanner and the Actian Data Intelligence is done via the HTTPS protocol (on default port: 443), in a secure way (TLS) and via an authentication based on an API key generated by you.
...
!!! warning "Required — The scanner must keep running"
    ...
    * Keep the scanner running continuously — during the execution of its sub-processes (by default, every night) and whenever users access the Actian Data Intelligence. For production environments, run the scanner as a managed service to ensure that it remains running and restarts automatically (see [Process Configuration in systemd](#process-configuration-in-systemd)).
...
* **synchronize**: Some connectors don't use an inventory to let the Data Steward import the items they want, but instead synchronize automatically all the items from the source system with Actian Data Intelligence. This process will trigger these connectors. By default, this process runs every 24H.
...
It is possible to configure a proxy that will be used by the Scanner during exchanges with the Actian Data Intelligence.
