...
!!! warning "Required — the scanner must keep running"
    ...
    **The Zeenea Scanner must be running for any metadata import or synchronization to work.** It is a long-running background process, not a one-off command.

    * Keep the scanner running **continuously** — during the execution of its sub-processes (by default every night) and the entire time users may use the Zeenea platform. For production, run it as a managed service so it stays up and restarts automatically (see *Process Configuration in systemd* below).
    * **On startup, the scanner immediately checks for and runs the jobs you triggered from the UI** (for example, an import you launched). It does **not** wait for the next scheduled time to pick those up.
    * **Scheduling** (the `automation` node) only triggers the recurring sync jobs without relying on the platform — it is not what makes a UI-triggered import run.

    **Symptom if the scanner is not running:** imports and other actions never complete and the UI stays on *"Your request has been taken into account"*. If you see this, first confirm the scanner process is actually running. See [Troubleshooting Scanners and Connections](./zeenea-troubleshooting.md).
