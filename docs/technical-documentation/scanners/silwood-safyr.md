...
Safyr must be purchased separately from your Actian Data Intelligence license.

Please contact our sales team if you need more information.

!!! note
    As Safyr can only be executed on Windows PC and Actian Data Intelligence Scanner is commonly installed on Linux instances, the files generated by Safyr must be to a location that the Scanner can access.
...
* To extend time and add functionality for the trial period:
    ...
    * Silwood will return a license key which will extend the trial duration, and enable options like export from SAP BW or the ability to export metadata from Safyr using Silwood Premium JSON file export format. This format will be used by Actian Data Intelligence to ingest the metadata into the data catalog.
