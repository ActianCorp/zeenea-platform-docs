...
A data source describes where data is physically located within an organization’s systems. It is neither a catalog object nor a graph node.

A data source is characterized by:

* A technical name
* A functional name (coming soon)
* An identifier
* Links to catalog objects

For V2 connectors:

* The technical name and identifier of the data source are generated by the connector from the connection information.
* The data source identifier is normalized and consists of:

     * A mandatory predefined type (for example, `snowflake`, `bigquery`)
     * Dynamic properties specific to the source system

For V1 connectors:

* The technical name corresponds to the connection name.
* The identifier corresponds to the connection code.

Multiple connections can reference the same data source. A single connection can also reference multiple data sources (for example, within a Snowflake account). For an external tool (for example, data quality tool), a single data source is defined.
...
Read more about it: [Creating, Editing, or deleting a property](../features-applications/studio/catalog-design/studio-create-edit-delete-property.md)
...
Read more: [Importing Datasets or Visualizations](../features-applications/studio/stewardship/importing-datasets-or-visualizations.md)
